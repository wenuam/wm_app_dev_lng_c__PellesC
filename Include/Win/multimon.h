...
/* Multiple monitor fake API definitions */
...
#endif /* !DISPLAY_DEVICE_ATTACHED_TO_DESKTOP */
...
HMONITOR WINAPI xMonitorFromPoint(POINT ptScreenCoords, DWORD dwFlags) {
    ...
    if ((dwFlags & (MONITOR_DEFAULTTOPRIMARY | MONITOR_DEFAULTTONEAREST)) ||
        ((ptScreenCoords.x >= 0) && (ptScreenCoords.x < GetSystemMetrics(SM_CXSCREEN)) &&
         (ptScreenCoords.y >= 0) && (ptScreenCoords.y < GetSystemMetrics(SM_CYSCREEN)))) {
...
HMONITOR WINAPI xMonitorFromRect(LPCRECT lprcScreenCoords, DWORD dwFlags) {
    ...
    if ((dwFlags & (MONITOR_DEFAULTTOPRIMARY | MONITOR_DEFAULTTONEAREST)) ||
        ((lprcScreenCoords->right  > 0) && (lprcScreenCoords->left < GetSystemMetrics(SM_CXSCREEN)) &&
         (lprcScreenCoords->bottom > 0) && (lprcScreenCoords->top  < GetSystemMetrics(SM_CYSCREEN)))) {
        return xPRIMARY_MONITOR;
    }
    return NULL;
}
...
BOOL WINAPI xGetMonitorInfo(HMONITOR hMonitor, _Inout_ LPMONITORINFO lpMonitorInfo) {
    RECT rcWork;
    if (InitMultipleMonitorStubs()) {
    ...
    if ((hMonitor == xPRIMARY_MONITOR) && lpMonitorInfo &&
        ...
        if (lpMonitorInfo->cbSize >= sizeof(MONITORINFOEX)) {
            ...
            MultiByteToWideChar(CP_ACP, 0, "DISPLAY", -1,
                ...
                (sizeof(((MONITORINFOEX *)lpMonitorInfo)->szDevice) / sizeof(TCHAR)));
#else /* !UNICODE */
            lstrcpyn(((MONITORINFOEX *)lpMonitorInfo)->szDevice, TEXT("DISPLAY"),
                (sizeof(((MONITORINFOEX *)lpMonitorInfo)->szDevice) / sizeof(TCHAR)));
#endif /* !UNICODE */
        }
        return TRUE;
...
BOOL WINAPI xEnumDisplayDevices(
    PVOID Unused,
    DWORD iDevNum,
    ...
    DWORD dwFlags)
{
    ...
    MultiByteToWideChar(CP_ACP, 0, "DISPLAY", -1, lpDisplayDevice->DeviceString,
        (sizeof(lpDisplayDevice->DeviceString) / sizeof(TCHAR)));
#else /* !UNICODE */
    lstrcpyn((LPTSTR)lpDisplayDevice->DeviceName, TEXT("DISPLAY"),
        (sizeof(lpDisplayDevice->DeviceName) / sizeof(TCHAR)));
    lstrcpyn((LPTSTR)lpDisplayDevice->DeviceString, TEXT("DISPLAY"),
        (sizeof(lpDisplayDevice->DeviceString) / sizeof(TCHAR)));
#endif /* !UNICODE */
