...
/* ApiSet contract for api-ms-win-netsec-isolation-l1 */
...
typedef void (CALLBACK *PAC_CHANGES_CALLBACK_FN)(
    void *context,
    const INET_FIREWALL_AC_CHANGE *pChange
);

HRESULT WINAPI NetworkIsolationSetupAppContainerBinaries(
...
DWORD WINAPI NetworkIsolationRegisterForAppContainerChanges(
    DWORD flags,
...
DWORD WINAPI NetworkIsolationUnregisterForAppContainerChanges(
    HANDLE registrationObject
);

DWORD WINAPI NetworkIsolationFreeAppContainers(
    PINET_FIREWALL_APP_CONTAINER pPublicAppCs
);

DWORD WINAPI NetworkIsolationEnumAppContainers(
    DWORD Flags,
    DWORD *pdwNumPublicAppCs,
    PINET_FIREWALL_APP_CONTAINER *ppPublicAppCs
);

DWORD WINAPI NetworkIsolationGetAppContainerConfig(
    DWORD *pdwNumPublicAppCs,
    PSID_AND_ATTRIBUTES *appContainerSids
);

DWORD WINAPI NetworkIsolationSetAppContainerConfig(
...
DWORD WINAPI NetworkIsolationDiagnoseConnectFailureAndGetInfo(
    LPCWSTR wszServerName,
