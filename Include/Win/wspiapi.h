...
#if __POCC__ >= 290
#pragma warn(push)
#pragma warn(disable:2228)  /* Unable to expand function 'x' inline */
...
inline char * WINAPI WspiapiStrdup(
    const char *pszString)
{
    ...
    pszMemory = WspiapiMalloc(cchMemory);
    if (!pszMemory)
        return NULL;
...
inline BOOL WINAPI WspiapiParseV4Address(
...
inline struct addrinfo * WINAPI WspiapiNewAddrInfo(
    int iSocketType,
    int iProtocol,
    WORD wPort,
    DWORD dwAddress)
{
    ...
    ptNew = WspiapiMalloc(sizeof(struct addrinfo));
    if (!ptNew)
        return NULL;

    ptAddress = WspiapiMalloc(sizeof(struct sockaddr_in));
    if (!ptAddress)
    {
        WspiapiFree(ptNew);
        return NULL;
...
inline int WINAPI WspiapiQueryDNS(
...
inline int WINAPI WspiapiLookupNode(
    const char *pszNodeName,
    int iSocketType,
    int iProtocol,
    WORD wPort,
...
inline int WINAPI WspiapiClone(
    WORD wPort,
...
inline void WINAPI WspiapiLegacyFreeAddrInfo(
    struct addrinfo *ptHead)
{
...
inline int WINAPI WspiapiLegacyGetAddrInfo(
    const char *pszNodeName,
...
inline int WINAPI WspiapiLegacyGetNameInfo(
...
inline FARPROC WINAPI WspiapiLoad(
    WORD wFunction)
{
...
inline int WINAPI WspiapiGetAddrInfo(
    const char *nodename,
    const char *servname,
    const struct addrinfo *hints,
    struct addrinfo * *res)
...
inline int WINAPI WspiapiGetNameInfo(
    const struct sockaddr *sa,
    socklen_t salen,
    char *host,
    size_t hostlen,
...
inline void WINAPI WspiapiFreeAddrInfo(
    struct addrinfo *ai)
{
