...
#if defined(_M_MRX000) || defined(_M_AMD64) || defined(_M_ARM64)
...
#define UNALIGNED64
#endif /* !_WIN64 */
#else /* !(defined(_M_MRX000) || defined(_M_AMD64) || defined(_M_ARM64)) */
#undef ALIGNMENT_MACHINE
#define UNALIGNED
#define UNALIGNED64
#endif /* !(defined(_M_MRX000) || defined(_M_AMD64) || defined(_M_ARM64)) */
...
#if (__POCC__ >= 650) && !defined(MIDL_PASS)
...
#if defined(MIDL_PASS) || defined(RC_INVOKED) || defined(_M_AMD64) || defined(_M_ARM64) || defined(_M_HYBRID_X86_ARM64)
...
ULONGLONG NTAPI Int64ShllMod32(
    ULONGLONG Value,
    DWORD ShiftCount
);

LONGLONG NTAPI Int64ShraMod32(
    LONGLONG Value,
    DWORD ShiftCount
);

ULONGLONG NTAPI Int64ShrlMod32(
    ULONGLONG Value,
    DWORD ShiftCount
);
...
_CRTIMP unsigned int __cdecl _rotl(unsigned int Value, int Shift);
unsigned __int64 __cdecl _rotl64(unsigned __int64 Value, int Shift);
_CRTIMP unsigned int __cdecl _rotr(unsigned int Value, int Shift);
...
#if __POCC__ >= 1000
#define FIELD_OFFSET(type,field)    ((LONG)__offsetof(type, field))
#define UFIELD_OFFSET(type,field)  ((DWORD)__offsetof(type, field))
#endif /* __POCC__ >= 1000 */
...
BOOLEAN __cdecl _interlockedbittestandset64(LONG64 volatile *Base, LONG64 Offset);
BOOLEAN __cdecl _interlockedbittestandreset64(LONG64 volatile *Base, LONG64 Offset);
...
SHORT InterlockedExchange16(SHORT volatile *Destination, SHORT ExChange);
...
char InterlockedExchangeAdd8(char volatile * _Addend, char _Value);
...
#else /* _M_ARM64EC */
__forceinline void __CpuId(int CPUInfo[4], int Function)
{
...
__forceinline unsigned __int64 __readmsr(unsigned long Register)
{
    (Register);
    __fastfail(0);
    return 0;
...
__forceinline void __writemsr(unsigned long Register, unsigned __int64 Value)
{
    (Register);
    (Value);
    __fastfail(0);
...
void __cdecl _mm_prefetch(CHAR CONST *a, int sel);
void __cdecl _m_prefetchw(volatile CONST VOID *Source);
#endif /* !_M_ARM64EC */
...
DWORD __cdecl __segmentlimit(DWORD Selector);
#pragma intrinsic(__segmentlimit)

#define ReadPMC  __readpmc

DWORD64 __cdecl __readpmc(DWORD Counter);
...
DWORD64 PopulationCount64(DWORD64 operand);
...
typedef PRUNTIME_FUNCTION GET_RUNTIME_FUNCTION_CALLBACK(
    DWORD64 ControlPc,
    PVOID Context
);
typedef GET_RUNTIME_FUNCTION_CALLBACK *PGET_RUNTIME_FUNCTION_CALLBACK;

typedef DWORD OUT_OF_PROCESS_FUNCTION_TABLE_CALLBACK(
    HANDLE Process,
    PVOID TableAddress,
    PDWORD Entries,
...
FORCEINLINE void WriteRelease(LONG volatile *Destination, LONG Value)
{
    __dmb(_ARM64_BARRIER_ISH);
...
FORCEINLINE void WriteRelease64(LONG64 volatile *Destination, LONG64 Value)
{
    __dmb(_ARM64_BARRIER_ISH);
...
FORCEINLINE void BarrierAfterRead(void)
{
    __dmb(_ARM64_BARRIER_ISH);
    return;
}
...
#ifndef PopulationCount64
__forceinline DWORD64 PopulationCount64(DWORD64 operand)
{
...
__forceinline DWORD64 ShiftLeft128(DWORD64 LowPart, DWORD64 HighPart, BYTE Shift)
{
    Shift &= 63;
    if (Shift == 0) {
        return HighPart;
...
__forceinline DWORD64 ShiftRight128(DWORD64 LowPart, DWORD64 HighPart, BYTE Shift)
{
    Shift &= 63;
    if (Shift == 0) {
        return LowPart;
...
#ifndef ARM64_MULT_INTRINSICS_SUPPORTED
#define ARM64_MULT_INTRINSICS_SUPPORTED  0
#endif /* ARM64_MULT_INTRINSICS_SUPPORTED */

#ifndef UnsignedMultiply128
__forceinline DWORD64 UnsignedMultiply128(DWORD64 Multiplier, DWORD64 Multiplicand,  DWORD64 *HighProduct)
{
...
__forceinline LONG64 Multiply128(LONG64 Multiplier, LONG64 Multiplicand, LONG64 *HighProduct)
{
...
typedef PARM64_RUNTIME_FUNCTION GET_RUNTIME_FUNCTION_CALLBACK(
    DWORD64 ControlPc,
    PVOID Context
);
typedef GET_RUNTIME_FUNCTION_CALLBACK *PGET_RUNTIME_FUNCTION_CALLBACK;
...
typedef DWORD OUT_OF_PROCESS_FUNCTION_TABLE_CALLBACK(
    ...
    PVOID TableAddress,
    PDWORD Entries,
    PARM64_RUNTIME_FUNCTION * Functions
);
typedef OUT_OF_PROCESS_FUNCTION_TABLE_CALLBACK *POUT_OF_PROCESS_FUNCTION_TABLE_CALLBACK;
...
FORCEINLINE PVOID _InlineInterlockedCompareExchangePointer(
...
charInterlockedExchangeAdd8(char volatile * _Addend, char _Value);

char InterlockedAnd8(char volatile *Destination, char Value);
char InterlockedOr8(char volatile *Destination, char Value);
char InterlockedXor8(char volatile *Destination, char Value);

SHORT __cdecl _InterlockedAnd16(SHORT volatile *Destination, SHORT Value);

SHORT InterlockedXor16(SHORT volatile *Destination, SHORT Value);

SHORT __cdecl _InterlockedCompareExchange16(SHORT volatile *Destination, SHORT ExChange, SHORT Comperand);
SHORT __cdecl _InterlockedOr16(SHORT volatile *Destination, SHORT Value);
SHORT __cdecl _InterlockedIncrement16(SHORT volatile *Destination);
SHORT __cdecl _InterlockedDecrement16(SHORT volatile *Destination);
...
LONG __cdecl _InterlockedXor(LONG volatile *Target, LONG Set);
...
void __cdecl __cpuidex(int CPUInfo[4], int Function, int SubLeaf);
#pragma intrinsic(__cpuidex)
...
void __cdecl_mm_prefetch(CHAR CONST *a, int sel);
...
#if !defined(RC_INVOKED) && !defined(MIDL_PASS)
#if ((defined(_M_AMD64) || defined(_M_IX86)) && !defined(_M_HYBRID_X86_ARM64) && !defined(_M_ARM64EC))
...
CFORCEINLINE LONG ReadNoFence(LONG const volatile *Source)
{
    LONG Value;
    Value = *Source;
    return Value;
...
CFORCEINLINE LONG64 ReadNoFence64(LONG64 const volatile *Source)
{
    LONG64 Value;
    Value = *Source;
    return Value;
...
#endif /* ((defined(_M_AMD64) || defined(_M_IX86)) && !defined(_M_HYBRID_X86_ARM64) && !defined(_M_ARM64EC)) */
...
FORCEINLINE BYTE ReadBooleanRaw(BOOLEAN const volatile *Source)
{
...
FORCEINLINE INT32 ReadInt32Acquire(INT32 const volatile *Source)
{
    return (INT32)ReadAcquire((PLONG)Source);
}

FORCEINLINE INT32 ReadInt32NoFence(INT32 const volatile *Source)
{
    return (INT32)ReadNoFence((PLONG)Source);
}

FORCEINLINE INT32 ReadInt32Raw(INT32 const volatile *Source)
{
    return (INT32)ReadRaw((PLONG)Source);
}

CFORCEINLINE void WriteInt32Release(INT32 volatile *Destination, INT32 Value)
{
    WriteRelease((PLONG)Destination, (LONG)Value);
    return;
}

FORCEINLINE void WriteInt32NoFence(INT32 volatile *Destination, INT32 Value)
{
    WriteNoFence((PLONG)Destination, (LONG)Value);
    return;
}

FORCEINLINE void WriteInt32Raw(INT32 volatile *Destination, INT32 Value)
{
    WriteRaw((PLONG)Destination, (LONG)Value);
    return;
}

FORCEINLINE UINT32 ReadUInt32Acquire(UINT32 const volatile *Source)
{
    return (UINT32)ReadAcquire((PLONG)Source);
}

FORCEINLINE UINT32 ReadUInt32NoFence(UINT32 const volatile *Source)
{
    return (UINT32)ReadNoFence((PLONG)Source);
}

FORCEINLINE UINT32 ReadUInt32Raw(UINT32 const volatile *Source)
{
    return (UINT32)ReadRaw((PLONG)Source);
}

CFORCEINLINE void WriteUInt32Release(UINT32 volatile *Destination, UINT32 Value)
{
    WriteRelease((PLONG)Destination, (LONG)Value);
    return;
}

FORCEINLINE void WriteUInt32NoFence(UINT32 volatile *Destination, UINT32 Value)
{
    WriteNoFence((PLONG)Destination, (LONG)Value);
    return;
}

FORCEINLINE void WriteUInt32Raw(UINT32 volatile *Destination, UINT32 Value)
{
    WriteRaw((PLONG)Destination, (LONG)Value);
    return;
}
...
CFORCEINLINE PVOID ReadPointerNoFence(PVOID const volatile *Source)
{
...
#else /* 1_MSC_EXTENSIONS */
    DWORD FiberData;
#endif /* 1_MSC_EXTENSIONS */
...
#endif /* (_WIN32_WINNT >= _WIN32_WINNT_WIN10_NI) */
...
#define IsReparseTagReserved(_tag)          (((_tag) & 0x40000000))
...
#endif /* (_WIN32_WINNT >= _WIN32_WINNT_WIN10_19H2) */
...
#else /* (_WIN32_WINNT < _WIN32_WINNT_WIN10_FE) */
...
DEFINE_GUID(GUID_PROCESSOR_FREQUENCY_LIMIT, 0x75b0ae3f, 0xbce0, 0x45a7, 0x8c, 0x89, 0xc9, 0x61, 0x1c, 0x25, 0xe1, 0x00);
DEFINE_GUID(GUID_PROCESSOR_FREQUENCY_LIMIT_1, 0x75b0ae3f, 0xbce0, 0x45a7, 0x8c, 0x89, 0xc9, 0x61, 0x1c, 0x25, 0xe1, 0x01);
...
DEFINE_GUID(GUID_PROCESSOR_PERF_ENERGY_PERFORMANCE_PREFERENCE_1, 0x36687f9e, 0xe3a5, 0x4dbf, 0xb1, 0xdc, 0x15, 0xeb, 0x38, 0x1c, 0x68, 0x64);
...
DEFINE_GUID(GUID_PROCESSOR_SOFT_PARKING_LATENCY, 0x97cfac41, 0x2217, 0x47eb, 0x99, 0x2d, 0x61, 0x8b, 0x19, 0x77, 0xc9, 0x7);
...
DEFINE_GUID(GUID_PROCESSOR_THREAD_SCHEDULING_POLICY, 0x93b8b6dc, 0x698, 0x4d1c, 0x9e, 0xe4, 0x6, 0x44, 0xe9, 0x0, 0xc8, 0x5d);
DEFINE_GUID(GUID_PROCESSOR_SHORT_THREAD_SCHEDULING_POLICY, 0xbae08b81, 0x2d5e, 0x4688, 0xad, 0x6a, 0x13, 0x24, 0x33, 0x56, 0x65, 0x4b);
DEFINE_GUID(GUID_PROCESSOR_SHORT_THREAD_RUNTIME_THRESHOLD, 0xd92998c2, 0x6a48, 0x49ca, 0x85, 0xd4, 0x8c, 0xce, 0xec, 0x29, 0x45, 0x70);
DEFINE_GUID(GUID_PROCESSOR_SHORT_THREAD_ARCH_CLASS_UPPER_THRESHOLD, 0x828423eb, 0x8662, 0x4344, 0x90, 0xf7, 0x52, 0xbf, 0x15, 0x87, 0x0f, 0x5a);
DEFINE_GUID(GUID_PROCESSOR_SHORT_THREAD_ARCH_CLASS_LOWER_THRESHOLD, 0x53824d46, 0x87bd, 0x4739, 0xaa, 0x1b, 0xaa, 0x79, 0x3f, 0xac, 0x36, 0xd6);
DEFINE_GUID(GUID_PROCESSOR_LONG_THREAD_ARCH_CLASS_UPPER_THRESHOLD, 0xbf903d33, 0x9d24, 0x49d3, 0xa4, 0x68, 0xe6, 0x5e, 0x03, 0x25, 0x04, 0x6a);
DEFINE_GUID(GUID_PROCESSOR_LONG_THREAD_ARCH_CLASS_LOWER_THRESHOLD, 0x43f278bc, 0x0f8a, 0x46d0, 0x8b, 0x31, 0x9a, 0x23, 0xe6, 0x15, 0xd7, 0x13);
DEFINE_GUID(GUID_SYSTEM_COOLING_POLICY, 0x94D3A615, 0xA899, 0x4AC5, 0xAE, 0x2B, 0xE4, 0xD8, 0xF6, 0x34, 0x36, 0x7F);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_DISABLE_THRESHOLD, 0x38b8383d, 0xcce0, 0x4c79, 0x9e, 0x3e, 0x56, 0xa4, 0xf1, 0x7c, 0xc4, 0x80);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_DISABLE_THRESHOLD_1, 0x38b8383d, 0xcce0, 0x4c79, 0x9e, 0x3e, 0x56, 0xa4, 0xf1, 0x7c, 0xc4, 0x81);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_ENABLE_THRESHOLD, 0x3d44e256, 0x7222, 0x4415, 0xa9, 0xed, 0x9c, 0x45, 0xfa, 0x3d, 0xd8, 0x30);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_ENABLE_THRESHOLD_1, 0x3d44e256, 0x7222, 0x4415, 0xa9, 0xed, 0x9c, 0x45, 0xfa, 0x3d, 0xd8, 0x31);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_DISABLE_TIME, 0xf565999f, 0x3fb0, 0x411a, 0xa2, 0x26, 0x3f, 0x1, 0x98, 0xde, 0xc1, 0x30);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_DISABLE_TIME_1, 0xf565999f, 0x3fb0, 0x411a, 0xa2, 0x26, 0x3f, 0x1, 0x98, 0xde, 0xc1, 0x31);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_ENABLE_TIME, 0x3d915188, 0x7830, 0x49ae, 0xa7, 0x9a, 0xf, 0xb0, 0xa1, 0xe5, 0xa2, 0x0);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_ENABLE_TIME_1, 0x3d915188, 0x7830, 0x49ae, 0xa7, 0x9a, 0xf, 0xb0, 0xa1, 0xe5, 0xa2, 0x1);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_EPP_CEILING, 0x4427c73b, 0x9756, 0x4a5c, 0xb8, 0x4b, 0xc7, 0xbd, 0xa7, 0x9c, 0x73, 0x20);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_EPP_CEILING_1, 0x4427c73b, 0x9756, 0x4a5c, 0xb8, 0x4b, 0xc7, 0xbd, 0xa7, 0x9c, 0x73, 0x21);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_PERF_FLOOR, 0xce8e92ee, 0x6a86, 0x4572, 0xbf, 0xe0, 0x20, 0xc2, 0x1d, 0x3, 0xcd, 0x40);
DEFINE_GUID(GUID_PROCESSOR_RESPONSIVENESS_PERF_FLOOR_1, 0xce8e92ee, 0x6a86, 0x4572, 0xbf, 0xe0, 0x20, 0xc2, 0x1d, 0x3, 0xcd, 0x41);
...
DEFINE_GUID(GUID_MIXED_REALITY_MODE, 0x1e626b4e, 0xcf04, 0x4f8d, 0x9c, 0xc7, 0xc9, 0x7c, 0x5b, 0xf, 0x23, 0x91);
DEFINE_GUID(GUID_SPR_ACTIVE_SESSION_CHANGE, 0xe24ce38, 0xc393, 0x4742, 0xbd, 0xb1, 0x74, 0x4f, 0x4b, 0x9e, 0xe0, 0x8e);
...
#endif /* (NTDDI_VERSION >= NTDDI_WINXP) || !defined(_BATCLASS_H) */
...
#define _IMAGE_DOS_HEADER_DEFINED  1
...
#define IMAGE_REL_AMD64_IMPORT_BR       0x0012
#define IMAGE_REL_AMD64_IMPORT_CALL     0x0013
#define IMAGE_REL_AMD64_CFG_BR          0x0014
#define IMAGE_REL_AMD64_CFG_BR_REX      0x0015
#define IMAGE_REL_AMD64_CFG_CALL        0x0016
#define IMAGE_REL_AMD64_INDIR_BR        0x0017
#define IMAGE_REL_AMD64_INDIR_BR_REX    0x0018
#define IMAGE_REL_AMD64_INDIR_CALL      0x0019
#define IMAGE_REL_AMD64_INDIR_BR_SWITCHTABLE_FIRST  0x0020
#define IMAGE_REL_AMD64_INDIR_BR_SWITCHTABLE_LAST   0x002F
...
#define IMAGE_FUNCTION_OVERRIDE_X64_REL32       1
#define IMAGE_FUNCTION_OVERRIDE_ARM64_BRANCH26  2
...
#define IMAGE_GUARD_CASTGUARD_PRESENT                  0x01000000
#define IMAGE_GUARD_MEMCPY_PRESENT                     0x02000000
...
NTSYSAPI void NTAPI RtlInitializeSListHead(
...
NTSYSAPI WORD NTAPI RtlQueryDepthSList (
    PSLIST_HEADER ListHead
);
...
#if __POCC__ >= 1000
DECLSPEC_NORETURN void __cdecl __fastfail(unsigned int Code);
...
#endif /* defined(_DBG_MEMCPY_INLINE_) && !defined(MIDL_PASS) && !defined(_MEMCPY_INLINE_) && !defined(_CRTBLD) */
...
#ifndef MIDL_PASS
FORCEINLINE int RtlConstantTimeEqualMemory(const void *v1, const void *v2, unsigned long len)
{
    char x = 0;
    unsigned long i = 0;
...
#if !defined(_WINBASE_H) && !defined(MIDL_PASS)
...
#endif /* !defined(_WINBASE_H) && !defined(MIDL_PASS) */
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
NTSYSAPI BOOLEAN NTAPI RtlGetProductInfo(
...
typedef void NTAPI RTL_UMS_SCHEDULER_ENTRY_POINT(
...
NTSYSAPI DWORD NTAPI RtlGetNonVolatileToken(
    PVOID NvBuffer,
    SIZE_T Size,
    PVOID *NvToken
);

NTSYSAPI DWORD NTAPI RtlFreeNonVolatileToken(
    PVOID NvToken
);

NTSYSAPI DWORD NTAPI RtlFlushNonVolatileMemory(
    PVOID NvToken,
    PVOID NvBuffer,
    SIZE_T Size,
    DWORD Flags
...
NTSYSAPI DWORD NTAPI RtlDrainNonVolatileFlush(
    PVOID NvToken
);

NTSYSAPI DWORD NTAPI RtlWriteNonVolatileMemory(
    PVOID NvToken,
...
NTSYSAPI DWORD NTAPI RtlFillNonVolatileMemory(
    PVOID NvToken,
    VOID UNALIGNED *NvDestination,
    SIZE_T Size,
    CONST BYTE Value,
...
NTSYSAPI DWORD NTAPI RtlFlushNonVolatileMemoryRanges(
    PVOID NvToken,
...
NTSYSAPI DWORD NTAPI RtlInitializeCorrelationVector(
    PCORRELATION_VECTOR CorrelationVector,
    int Version,
    const GUID * Guid
);

NTSYSAPI DWORD NTAPI RtlIncrementCorrelationVector(
    PCORRELATION_VECTOR CorrelationVector
);

NTSYSAPI DWORD NTAPI RtlExtendCorrelationVector(
    PCORRELATION_VECTOR CorrelationVector
);

NTSYSAPI DWORD NTAPI RtlValidateCorrelationVector(
...
#ifndef MIDL_PASS
FORCEINLINE void CUSTOM_SYSTEM_EVENT_TRIGGER_INIT(
...
DWORD NTAPI RtlRaiseCustomSystemEventTrigger(
...
NTSYSAPI BOOLEAN NTAPI RtlNormalizeSecurityDescriptor(
    PSECURITY_DESCRIPTOR *SecurityDescriptor,
...
NTSYSAPI DWORD NTAPI RtlGetSystemGlobalData(
    RTL_SYSTEM_GLOBAL_DATA_ID DataId,
    PVOID Buffer,
    DWORD Size
);

NTSYSAPI DWORD NTAPI RtlSetSystemGlobalData(
    RTL_SYSTEM_GLOBAL_DATA_ID DataId,
    PVOID Buffer,
    DWORD Size
);
...
void NTAPI RtlGetDeviceFamilyInfoEnum(
    ULONGLONG *pullUAPInfo,
    DWORD *pulDeviceFamily,
    DWORD *pulDeviceForm
);

DWORD NTAPI RtlConvertDeviceFamilyInfoToString(
...
FORCEINLINE void TpInitializeCallbackEnviron(
...
FORCEINLINE void TpSetCallbackThreadpool(
    PTP_CALLBACK_ENVIRON CallbackEnviron,
    PTP_POOL Pool)
{
...
FORCEINLINE void TpSetCallbackCleanupGroup(
    PTP_CALLBACK_ENVIRON CallbackEnviron,
    PTP_CLEANUP_GROUP CleanupGroup,
    PTP_CLEANUP_GROUP_CANCEL_CALLBACK CleanupGroupCancelCallback)
{
...
FORCEINLINE void TpSetCallbackNoActivationContext(
    PTP_CALLBACK_ENVIRON CallbackEnviron)
{
    CallbackEnviron->ActivationContext = (struct _ACTIVATION_CONTEXT *)(LONG_PTR) -1;
}

FORCEINLINE void TpSetCallbackLongFunction(
    PTP_CALLBACK_ENVIRON CallbackEnviron)
{
    CallbackEnviron->u.s.LongFunction = 1;
}

FORCEINLINE void TpSetCallbackRaceWithDll(
    PTP_CALLBACK_ENVIRON CallbackEnviron,
    PVOID DllHandle)
{
...
FORCEINLINE void TpSetCallbackFinalizationCallback(
    PTP_CALLBACK_ENVIRON CallbackEnviron,
    PTP_SIMPLE_CALLBACK FinalizationCallback)
{
...
#if (_WIN32_WINNT >= _WIN32_WINNT_WIN7)
FORCEINLINE void TpSetCallbackPriority(
    PTP_CALLBACK_ENVIRON CallbackEnviron,
...
FORCEINLINE void TpSetCallbackPersistent(
    PTP_CALLBACK_ENVIRON CallbackEnviron)
{
    CallbackEnviron->u.s.Persistent = 1;
}

FORCEINLINE void TpDestroyCallbackEnviron(
    PTP_CALLBACK_ENVIRON CallbackEnviron)
{
...
#endif /* defined(_M_AMD64) && !defined(_M_ARM64EC) && !defined(__midl) */

#if (defined(_M_ARM64) || defined(_M_ARM64EC)) && !defined(__midl)
...
#endif /* (defined(_M_ARM64) || defined(_M_ARM64EC)) && !defined(__midl) */
