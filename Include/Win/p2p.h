...
#if __POCC__ >= 200
#pragma message("the peer-to-peer infrastructure is deprecated")
#endif
...
HRESULT WINAPI PeerGraphCreate(
    ...
    PCWSTR pwzDatabaseName,
    PPEER_SECURITY_INTERFACE pSecurityInterface,
    HGRAPH *phGraph);
...
HRESULT WINAPI PeerGraphOpen(
    ...
    PCWSTR pwzDatabaseName,
    PPEER_SECURITY_INTERFACE pSecurityInterface,
    ULONG cRecordTypeSyncPrecedence,
    const GUID *pRecordTypeSyncPrecedence,
    HGRAPH *phGraph);
...
HRESULT WINAPI PeerGraphGetProperties(
    HGRAPH hGraph,
    PPEER_GRAPH_PROPERTIES *ppGraphProperties);
...
HRESULT WINAPI PeerGraphSetNodeAttributes(
    HGRAPH hGraph,
    PCWSTR pwzAttributes);

HRESULT WINAPI PeerGraphPeerTimeToUniversalTime(
    HGRAPH hGraph,
    FILETIME *pftPeerTime,
    FILETIME *pftUniversalTime);
...
#endif /* !MIDL_PASS */
#endif /* (!defined(NO_P2P_GROUP) || !defined(NO_P2P_IDENTITY)) */
...
HRESULT WINAPI PeerGroupRegisterEvent(
    ...
    PEER_GROUP_EVENT_REGISTRATION *pEventRegistrations,
    HPEEREVENT *phPeerEvent);
...
HRESULT WINAPI PeerCollabGetApplicationRegistrationInfo(
    const GUID *pApplicationId,
    PEER_APPLICATION_REGISTRATION_TYPE registrationType,
    PPEER_APPLICATION_REGISTRATION_INFO *ppApplication);
...
HRESULT WINAPI PeerCollabSubscribeEndpointData(
    const PCPEER_ENDPOINT pcEndpoint);
...
HRESULT WINAPI PeerCollabRegisterEvent(
    HANDLE hEvent,
    DWORD cEventRegistration,
    PEER_COLLAB_EVENT_REGISTRATION *pEventRegistrations,
    HPEEREVENT *phPeerEvent);
...
HRESULT WINAPI PeerPnrpRegister(
    PCWSTR pcwzPeerName,
    PPEER_PNRP_REGISTRATION_INFO pRegistrationInfo,
    HREGISTRATION *phRegistration);
...
HRESULT WINAPI PeerPnrpResolve(
    PCWSTR pcwzPeerName,
    PCWSTR pcwzCloudName,
    ULONG *pcEndpoints,
    PPEER_PNRP_ENDPOINT_INFO *ppEndpoints);
...
HRESULT WINAPI PeerPnrpGetEndpoint(
    HRESOLUTION hResolve,
    PPEER_PNRP_ENDPOINT_INFO *ppEndpoint);
