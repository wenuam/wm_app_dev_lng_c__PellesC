...
typedef HRESULT (CALLBACK *WS_ENCODER_GET_CONTENT_TYPE_CALLBACK)(
    void *encoderContext,
    const WS_STRING *contentType,
    WS_STRING *newContentType,
    WS_STRING *contentEncoding,
    WS_ERROR *error);
...
typedef HRESULT (CALLBACK *WS_GET_CERT_CALLBACK)(
    ...
    const WS_ENDPOINT_ADDRESS *targetAddress,
    const WS_STRING *viaUri,
...
typedef HRESULT (CALLBACK *WS_CERT_ISSUER_LIST_NOTIFICATION_CALLBACK)(
    ...
    const struct _SecPkgContext_IssuerListInfoEx *issuerList,
    WS_ERROR *error);
...
typedef HRESULT (CALLBACK *WS_VALIDATE_SAML_CALLBACK)(
    ...
    WS_XML_BUFFER *samlAssertion,
    WS_ERROR *error);

typedef HRESULT (CALLBACK *WS_DURATION_COMPARISON_CALLBACK)(
    const WS_DURATION *duration1,
    const WS_DURATION *duration2,
    int *result,
    WS_ERROR *error);
...
typedef void (CALLBACK *WS_OPERATION_CANCEL_CALLBACK)(
    const WS_SERVICE_CANCEL_REASON reason,
    void *state);
...
HRESULT WINAPI WsRemoveNode(
    const WS_XML_NODE_POSITION *nodePosition,
    WS_ERROR *error);
...
void WINAPI WsFreeReader(WS_XML_READER *reader);
...
void WINAPI WsFreeWriter(WS_XML_WRITER *writer);
...
HRESULT WINAPI WsVerifyXmlNCName(
    ...
    ULONG ncNameCharCount,
    WS_ERROR *error);

HRESULT WINAPI WsXmlStringEquals(
    const WS_XML_STRING *string1,
    const WS_XML_STRING *string2,
    WS_ERROR *error);
...
HRESULT WINAPI WsSendMessage(
    WS_CHANNEL *channel,
    WS_MESSAGE *message,
    const WS_MESSAGE_DESCRIPTION *messageDescription,
...
void WINAPI WsFreeChannel(WS_CHANNEL *channel);
...
HRESULT WINAPI WsResetError(WS_ERROR *error);

void WINAPI WsFreeError(WS_ERROR *error);
...
void WINAPI WsFreeHeap(WS_HEAP *heap);
...
void WINAPI WsFreeListener(WS_LISTENER *listener);
...
void WINAPI WsFreeMessage(WS_MESSAGE *message);
...
HRESULT WINAPI WsGetCustomHeader(
    WS_MESSAGE *message,
    const WS_ELEMENT_DESCRIPTION *customHeaderDescription,
...
HRESULT WINAPI WsRemoveCustomHeader(
    WS_MESSAGE *message,
    const WS_XML_STRING *headerName,
    const WS_XML_STRING *headerNs,
    WS_ERROR *error);

HRESULT WINAPI WsAddCustomHeader(
    WS_MESSAGE *message,
    const WS_ELEMENT_DESCRIPTION *headerDescription,
    WS_WRITE_OPTION writeOption,
    const void *value,
    ULONG valueSize,
    ULONG headerAttributes,
...
HRESULT WINAPI WsWriteBody(
    WS_MESSAGE *message,
    const WS_ELEMENT_DESCRIPTION *bodyDescription,
    WS_WRITE_OPTION writeOption,
    const void *value,
    ULONG valueSize,
    WS_ERROR *error);
...
HRESULT WINAPI WsWriteEnvelopeStart(
    WS_MESSAGE *message,
    WS_XML_WRITER *writer,
    WS_MESSAGE_DONE_CALLBACK doneCallback,
...
HRESULT WINAPI WsMarkHeaderAsUnderstood(
    WS_MESSAGE *message,
    const WS_XML_NODE_POSITION *headerPosition,
    WS_ERROR *error);
...
void WINAPI WsFreeSecurityToken(WS_SECURITY_TOKEN *token);

HRESULT WINAPI WsRevokeSecurityContext(
    WS_SECURITY_CONTEXT *securityContext,
    WS_ERROR *error);
...
HRESULT WINAPI WsReadElement(
    WS_XML_READER *reader,
    const WS_ELEMENT_DESCRIPTION *elementDescription,
    WS_READ_OPTION readOption,
    WS_HEAP *heap,
    void *value,
    ULONG valueSize,
...
HRESULT WINAPI WsReadAttribute(
    WS_XML_READER *reader,
    const WS_ATTRIBUTE_DESCRIPTION *attributeDescription,
    WS_READ_OPTION readOption,
    WS_HEAP *heap,
    void *value,
    ULONG valueSize,
...
HRESULT WINAPI WsRegisterOperationForCancel(
    ...
    WS_OPERATION_FREE_STATE_CALLBACK freestateCallback,
    void *userState,
    WS_ERROR *error);

HRESULT WINAPI WsGetServiceHostProperty(
    WS_SERVICE_HOST *serviceHost,
    const WS_SERVICE_PROPERTY_ID id,
    void *value,
    ULONG valueSize,
    WS_ERROR *error);
...
void WINAPI WsFreeServiceHost(WS_SERVICE_HOST *serviceHost);
...
HRESULT WINAPI WsGetServiceProxyProperty(
    ...
    const WS_PROXY_PROPERTY_ID id,
    void *value,
    ULONG valueSize,
    WS_ERROR *error);

HRESULT WINAPI WsCreateServiceProxy(
    const WS_CHANNEL_TYPE channelType,
    const WS_CHANNEL_BINDING channelBinding,
    const WS_SECURITY_DESCRIPTION *securityDescription,
    const WS_PROXY_PROPERTY *properties,
    const ULONG propertyCount,
    const WS_CHANNEL_PROPERTY *channelProperties,
    const ULONG channelPropertyCount,
    WS_SERVICE_PROXY ** serviceProxy,
    WS_ERROR *error);
...
void WINAPI WsFreeServiceProxy(WS_SERVICE_PROXY *serviceProxy);
...
HRESULT WINAPI WsCombineUrl(
    const WS_STRING *baseUrl,
    const WS_STRING *referenceUrl,
    ULONG flags,
    WS_HEAP *heap,
    WS_STRING *resultUrl,
    WS_ERROR *error);
...
void WINAPI WsFreeMetadata(WS_METADATA *metadata);
