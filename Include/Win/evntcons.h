...
#define EVENT_HEADER_FLAG_DECODE_GUID           0x0080
...
EVNTCONS_INLINE ULONG GetEventProcessorIndex(PCEVENT_RECORD EventRecord) {
...
static inline size_t __STRNLEN(PUCHAR Ptr, size_t Max) {
    const UCHAR *PtrEnd = memchr(Ptr,'\0', Max);
    return (PtrEnd != 0 ? (PtrEnd - Ptr) : Max);
}

static inline USHORT __LOAD_USHORT(const void *Ptr) {
    const BYTE *BytePtr = Ptr;
    return (USHORT)BytePtr[0] << 0 |
           (USHORT)BytePtr[1] << 8;
}
...
EVNTCONS_INLINE void EtwGetTraitFromProviderTraits(
    ...
    PUSHORT Size)
{
    USHORT ByteCount = __LOAD_USHORT(ProviderTraits);
    PUCHAR Ptr = ProviderTraits;
    ...
    *Size = 0;
    ...
    Ptr += 2;
    Ptr += __STRNLEN(Ptr, ByteCount - 3u);
    Ptr += 1;
    while (Ptr < PtrEnd) {
        USHORT TraitByteCount = __LOAD_USHORT(Ptr);
        if (TraitByteCount < 3) {
            return;
        }
