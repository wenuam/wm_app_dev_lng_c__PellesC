...
/* Certificate Server backup client API definitions */
...
HRESULT CERTBCLI_API CertSrvBackupGetDatabaseNamesW(
    HCSBC hbc,
    PWSTR *ppwszzAttachmentInformation,
    DWORD *pcbSize
);

typedef HRESULT (CERTBCLI_CALL FNCERTSRVBACKUPOPENFILEW)(
    HCSBC hbc,
    WCHAR const *pwszAttachmentName,
...
HRESULT CERTBCLI_API CertSrvRestoreGetDatabaseLocationsW(
    HCSBC hbc,
    PWSTR *ppwszzDatabaseLocationList,
    DWORD *pcbSize
);
...
typedef HRESULT (CERTBCLI_CALL FNCERTSRVRESTOREREGISTERW)(
    HCSBC hbc,
    WCHAR const *pwszCheckPointFilePath,
    WCHAR const *pwszLogPath,
    CSEDB_RSTMAPW rgrstmap[],
    LONG crstmap,
    WCHAR const *pwszBackupLogPath,
    ULONG genLow,
    ULONG genHigh
);
