...
typedef struct IWCNDeviceVtbl {
    ...
    HRESULT (STDMETHODCALLTYPE *GetNetworkProfile )(
        IWCNDevice *This,
        DWORD cchMaxStringLength,
        LPWSTR wszProfile);

    HRESULT (STDMETHODCALLTYPE *SetNetworkProfile )(
        IWCNDevice *This,
        LPCWSTR pszProfileXml);

    HRESULT (STDMETHODCALLTYPE *GetVendorExtension )(
        IWCNDevice *This,
        const WCN_VENDOR_EXTENSION_SPEC *pVendorExtSpec,
        DWORD dwMaxBufferSize,
        BYTE pbBuffer[ ],
        DWORD *pdwBufferUsed);
    ...
    HRESULT (STDMETHODCALLTYPE *SetNFCPasswordParams )(
        ...
        const BYTE pbRemotePublicKeyHash[ ],
        DWORD dwDHKeyBlobLength,
        const BYTE pbDHKeyBlob[ ]);
