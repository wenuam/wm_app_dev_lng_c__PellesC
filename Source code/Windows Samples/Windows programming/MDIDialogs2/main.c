...
static void MdiChildWnd_OnMDIActivate(HWND, BOOL, HWND, HWND);
...
BOOL WinMain_Init(HINSTANCE hInst)
{
	...
	wc.lpszClassName = "MDIDialogs2Class";
...
LRESULT FrameWnd_OnCreate(HWND hwnd, LPCREATESTRUCT lParam)
{
	...
	g_hWndClient = ClientWnd_Create(hwnd, g_hInst);
	...
	g_hWndStatusBar = CreateStatusBarWnd(hwnd, "Ready", numParts);
	g_hWndToolBar = CreateToolBarWnd(hwnd, g_hInst);
...
LRESULT FrameWnd_OnSize(HWND hwnd, int flag, int x, int y)
{
	...
	if (flag != SIZE_MINIMIZED)
	{
		...
		MoveWindow(g_hWndClient, rcClient.left, rcClient.top, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top, TRUE);
	}
	return 0;
}
...
LRESULT FrameWnd_OnMenuSelect(HWND hwnd, HMENU hMenu, int uItem, HMENU hSubMenu, UINT fuFlags)
{
	...
	SetStatusBarText(g_hWndStatusBar, 0, szBuffer);
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	if ((g_hWndFrame = FrameWnd_Create(g_hInst)) == (HWND)0)
		return 0;
...
HWND FrameWnd_Create(HINSTANCE hInst)
{
	return CreateWindowEx(WS_EX_WINDOWEDGE | WS_EX_ACCEPTFILES, 
		"MDIDialogs2Class", 
		"MDIDialogs2 Program", 
...
static BOOL MdiChildWnd_OnCreate(HWND hwnd, CREATESTRUCT *pcs)
{
        ...
        (pDlg = LockResource(hgRes)) != NULL)
    {
        ...
        if (hwndDlg != NULL)
        {
            ...
            // Resize the MDI window to *roughly* the size of the dialog (we DO  account for borders).
