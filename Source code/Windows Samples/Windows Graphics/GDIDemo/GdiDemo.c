...
int PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpszLine, int nShow)
{
     ...
     */
    msg.wParam = 1;
    if ((hWndFrame = CreateAppWindow(hInst)) != NULL) {
...
LRESULT CALLBACK WndProc(HWND hWndFrame, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    HWND hWndClient;
...
BOOL CreateProc(HWND hWndFrame)
{
     ...
     */
    if ((hWndMDIClient = CreateMDIClientWindow(hWndFrame)) != NULL)
        SetWindowLongPtr(hWndFrame, CLIENTWND, (LONG_PTR)hWndMDIClient);
...
BOOL CommandProc(HWND hWndFrame, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_DEMO_POLYBEZIER:
        if ((hWndClient = (HWND)GetWindowLongPtr(hWndFrame, CLIENTWND)) != NULL)
...
VOID PaintProc(HWND hWndFrame)
{
    ...
    if ((hDC = BeginPaint(hWndFrame, &ps)) != NULL)
