...
HWND FAR CreateMazeWindow(HWND hWndClient, int nItem)
{
    ...
    LoadString(hInstance, MAZETITLE, buffer, 256);
...
/*---------------------------------------------------------------------------*\
   ...
   \*--------------------------------------------------------------------------- */
LRESULT CALLBACK MazeProc(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    switch (wMsg) {
    case WM_CREATE:
        MazeCreateProc(hWnd);
...
BOOL MazeCreateProc(HWND hWnd)
{
    ...
    if (AllocWindowInfo(hWnd, sizeof(MAZEDATA))) {
        if ((ppd = (PMAZEDATA)LockWindowInfo(hWnd)) != NULL) {
...
/*---------------------------------------------------------------------------*\
   ...
   \*--------------------------------------------------------------------------- */
BOOL MazeCommandProc(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    hWnd = hWnd;
    wParam = wParam;
    lParam = lParam;
