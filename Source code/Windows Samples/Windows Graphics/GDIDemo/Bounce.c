...
HWND FAR CreateBounceWindow(HWND hWndClient, int nItem)
{
    ...
    LoadString(hInstance, BOUNCETITLE, buffer, 256);
...
/*---------------------------------------------------------------------------*\
   ...
   |
   \*--------------------------------------------------------------------------- */
LRESULT CALLBACK BounceProc(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    switch (wMsg) {
    case WM_CREATE:
...
BOOL BounceCreateProc(HWND hWnd)
{
    ...
    if (AllocWindowInfo(hWnd, sizeof(BOUNCEDATA))) {
        if ((pbd = (PBOUNCEDATA)LockWindowInfo(hWnd)) != NULL) {
...
/*---------------------------------------------------------------------------*\
   ...
   |
   \*--------------------------------------------------------------------------- */
BOOL BounceCommandProc(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    hWnd = hWnd;
    wParam = wParam;
    lParam = lParam;
...
HANDLE ballCreate(HWND hWnd, int nWidth, int nHeight, COLORREF crColor)
{
    ...
    if ((hBall = LocalAlloc(LHND, sizeof(BALLDATA))) != NULL) {
        if ((pbd = (PBALLDATA)LocalLock(hBall)) != NULL) {
            pbd->hWnd = hWnd;
            pbd->nWidth = nWidth;
            pbd->nHeight = nHeight;
            pbd->xDirection = 0;
