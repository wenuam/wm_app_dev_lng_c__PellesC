...
/*---------------------------------------------------------------------------*\
   ...
   |
   \*--------------------------------------------------------------------------- */
INT_PTR FAR DisplayDialogBox(HWND hWnd, LPSTR lpszTemplate, WNDPROC lpfFunction, LONG lExtra)
{
    HANDLE hInstance;
    WNDPROC lpfDlg;
    INT_PTR nRet;
    ...
    nRet = -1;
    if ((hInstance = GETINSTANCE(hWnd)) != NULL) {
        if ((lpfDlg = MakeProcInstance(lpfFunction, hInstance)) != NULL) {
...
VOID PaintWindow(HWND hWnd, int nColor)
{
    ...
    if ((hDC = BeginPaint(hWnd, &ps)) != NULL) {
        ...
        if (GetDeviceCaps(hDC, RASTERCAPS) & RC_PALETTE) {
            ...
            if ((hPal = CreateColorScalePalette(hDC, nColor)) != NULL) {
...
HPALETTE CreateColorScalePalette(HDC hDC, int nColor)
{
    ...
    if (GetDeviceCaps(hDC, RASTERCAPS) & RC_PALETTE) {
        ...
        if ((hMem = GlobalAlloc(GHND, (DWORD)sizeof(LOGPALETTE) + (sizeof(PALETTEENTRY) * nSize))) != NULL) {
            if ((lpMem = (LPLOGPALETTE)GlobalLock(hMem)) != NULL) {
