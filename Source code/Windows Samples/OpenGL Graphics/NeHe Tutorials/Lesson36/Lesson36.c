...
GLuint EmptyTexture(void)										// Create An Empty Texture
{
	...
	data = malloc((128 * 128) * 4 * sizeof(unsigned int));
...
void ProcessHelix(void)											// Draws A Helix
{
	...
	for(phi=0; phi <= 360; phi+=20.0)							// 360 Degrees In Steps Of 20
	{
		for(theta=0; theta<=360*twists; theta+=20.0)			// 360 Degrees * Number Of Twists In Steps Of 20
		{
			...
			x=cosf(u)*(2.0f+cosf(v))*r;					// Calculate x Position (1st Point)
			y=sinf(u)*(2.0f+cosf(v))*r;					// Calculate y Position (1st Point)
			z=((u-(2.0f*3.142f))+sinf(v)) * r;			// Calculate z Position (1st Point)
			...
			x=cosf(u)*(2.0f+cosf(v))*r;					// Calculate x Position (2nd Point)
			y=sinf(u)*(2.0f+cosf(v))*r;					// Calculate y Position (2nd Point)
			z=((u-(2.0f*3.142f))+sinf(v))*r;				// Calculate z Position (2nd Point)
			...
			x=cosf(u)*(2.0f+cosf(v))*r;					// Calculate x Position (3rd Point)
			y=sinf(u)*(2.0f+cosf(v))*r;					// Calculate y Position (3rd Point)
			z=((u-(2.0f*3.142f))+sinf(v))*r;				// Calculate z Position (3rd Point)
			...
			x=cosf(u)*(2.0f+cosf(v))*r;					// Calculate x Position (4th Point)
			y=sinf(u)*(2.0f+cosf(v))*r;					// Calculate y Position (4th Point)
			z=((u-(2.0f*3.142f))+sinf(v))*r;				// Calculate z Position (4th Point)
...
void RenderToTexture(void)										// Renders To A Texture
{
