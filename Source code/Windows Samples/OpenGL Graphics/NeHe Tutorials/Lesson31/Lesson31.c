...
struct Model *pModel = NULL;										// Holds The Model Data
...
{
	Model_ReloadTextures(pModel);									// Loads Model Textures
...
int DrawGLScene(GLvoid)												// Here's Where We Do All The Drawing
{
	...
	Model_Draw(pModel);												// Draw The Model
					...
					int			nCmdShow)							// Window Show State
{
	...
	pModel = malloc(sizeof(struct Model));							// Memory To Hold The Model
	Model_Create(pModel);
	if ( MilkshapeModel_LoadModelData(pModel, "data/model.ms3d" ) == false )		// Loads The Model And Checks For Errors
	{
