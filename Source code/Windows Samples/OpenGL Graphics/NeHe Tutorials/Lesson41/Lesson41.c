...
int BuildTexture(char *szPathName, GLuint *texid)						// Load Image And Convert To A Texture
{
	...
	{
		pPicture->lpVtbl->Release(pPicture);							// Decrements IPicture Reference Count
		return FALSE;													// Return False (Failure)
	}

	glGetIntegerv(GL_MAX_TEXTURE_SIZE, &glMaxTexDim);					// Get Maximum Texture Size Supported
	
	pPicture->lpVtbl->get_Width(pPicture, &lWidth);						// Get IPicture Width (Convert To Pixels)
	lWidthPixels	= MulDiv(lWidth, GetDeviceCaps(hdcTemp, LOGPIXELSX), 2540);
	pPicture->lpVtbl->get_Height(pPicture, &lHeight);					// Get IPicture Height (Convert To Pixels)
	...
	pPicture->lpVtbl->Render(pPicture, hdcTemp, 0, 0, lWidthPixels, lHeightPixels, 0, lHeight, lWidth, -lHeight, 0);
