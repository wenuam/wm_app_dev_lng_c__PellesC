...
//
// PellesC version 0.01 (PK4BCX2  -  2004)
         ...
         if(!GetNextColWithWidth(SI,c,1))
             {
              //repaint right side of grid
                 ...
                 holdbrush=(HBRUSH)SelectObject(gdc,GetStockObject(WHITE_BRUSH));
...
/* IMPROVED VERSION - handles extended characters. Thanks Timppa! */
unsigned char GetASCII(WPARAM wParam, LPARAM lParam)
{
	int returnvalue;
	// char mbuffer[100];
	int result;
	...
	returnvalue = (unsigned char) dwReturnedValue;
	if (returnvalue < 0) {
		returnvalue = 0;
	}
	...
	return (unsigned char)returnvalue;
}
   ...
   // static int ASCII;
	...
	switch (message) 
	{
        ...
        case BGM_DELETECELL:
            ...
            break;
		case BGM_SETGRIDDIM:
			   if(((int)wParam>=0) && ((int)wParam<=MAX_ROWS))
			   {
		...
		case BGM_SETCOLWIDTH:
			  if(((int)wParam <= MAX_COLS)&&((int)wParam >= 0) && (lParam >= 0))
			  {
                RECT rect;
				  ...
				  DrawCursor(hWnd,SelfIndex);

                      {
                        int ascii;
                          ascii=GetASCII(wParam,lParam);
                          wParam = ascii;
                          if((wParam >= 32)&&(wParam <= 255 /*125*/))  /* PO: accept extended characters */
                              {
                               char tstring[2];
                               if(!BGHS[SelfIndex].EDITING)
                                   {
              ...
              if(CountGrids()==1)
                  {
                   ...
                   hfontheader=CreateFont(15,0,0, 0,FW_MEDIUM,FALSE,FALSE,FALSE,ANSI_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,VARIABLE_PITCH|FF_MODERN ,NULL);
                   hfonttitle=CreateFont(18,0,0, 0,FW_BOLD,FALSE,FALSE,FALSE,ANSI_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,VARIABLE_PITCH|FF_MODERN ,NULL);
                  }
