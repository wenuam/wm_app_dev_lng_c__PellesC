...
int DetermineDataType(char* data)
{
	...
	for(j=0;j<k;j++)
	{
		if(isalpha(tbuffer[j]))
		{
			ALPHA=TRUE;
		}
		if(isdigit(tbuffer[j]))
		{
			DIGIT=TRUE;
		}
		if(iswspace(tbuffer[j]))
		{
			WHITESPACE=TRUE;
		}
		if(tbuffer[j]=='.')
...
ATOM RegisterGridClass(HINSTANCE hInstance)
{
	...
	wcx.lpfnWndProc=(WNDPROC)GridProc;
	wcx.lpszClassName="BABYGRID";
...
static LRESULT CALLBACK GridProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case BGM_SETCOLWIDTH:
			if(((int)wParam <= MAX_COLS)&&((int)wParam >= 0) && ((int)lParam >= 0))
			{
				RECT rect;
		...
		case WM_GETDLGCODE:
			...
			ReturnValue = DLGC_WANTARROWS|DLGC_WANTCHARS;
			// 	find and compare.
			if(wParam == 13)
			{
				//same as arrow down
