 ...
 * File    : main.c                                                         *
 *                                                                          *
 * Purpose : SQL Shell using ListView                                       *
 *                                                                          *
 * History : Date      Reason                                               *
 *           00/00/00  Created   Just For Fun                               *
 * Author  : Ottonello, Santiago sanotto@gmailc.om                          *
 *                                                                          *
 * Warning : Im just learning windows API programming, so ... be mercifull  *
 *           ... and ... do not ask advanced questions ;-)                  *
 *                                                                          *
 ****************************************************************************/
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	//Save Instance Handler to Global var to make it easily accesible
	hInst=hInstance;
  ...
  connection = (lpszCmdLine != NULL) ? lpszCmdLine : NULL;
  ...
  TRYODBC(dbc_handle, SQL_HANDLE_DBC, SQLAllocHandle(SQL_HANDLE_STMT, dbc_handle, &stmt_handle));
	...
	wcx.lpszClassName = _T("TestGUIClass");
	//Registrar la clase
	if (!RegisterClassEx(&wcx)) return 0;
	//Cargar Tabla de Aceleradores
	hAccel=LoadAccelerators(hInstance,MAKEINTRESOURCE(ACCEL_TAB));
	//Crear el Dialogo
	...
	{
		//Traducir teclazos si los hay
		TranslateAccelerator(hwnd,hAccel,&Msg);
		//Pedir proceso de msg para un dialogo
  ...
  switch (uMsg)
    {
        ...
        case WM_SIZE:
						...
						MoveWindow(GetDlgItem(hwndDlg,	SQL_SENTENCE ),	2,4,nWidth-4, 70, TRUE);	
						MoveWindow(GetDlgItem(hwndDlg,	DLG_TABLE),2,80,nWidth-4,nHeight -100, TRUE);	
						MoveWindow(GetDlgItem(hwndDlg,	DLG_STSBAR),0 ,nHeight-51,nWidth,	50, TRUE);	
            return TRUE;
